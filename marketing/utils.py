...
def get_subscriber_hash(member_email):
    checked_member_email = check_email(member_email)
    member_email = checked_member_email.lower().encode()
...
class Mailchimp(object):
    ...
    def change_subscription_status(self, email, status='unsubscribed', check_status='False'):
        ...
        # if check_status:
            # return requests.get(endpoint, auth=("", self.key), data=json.dumps(data))
    ...
    def add_email(self, email):  # Same as subscribed method.
        status = "subscribed"
